...
var viewProductSales = function () {
    ...
    connection.query('SELECT * FROM totalprofits', function (err, res) {
        ...
        for (var i = 0; i < res.length; i++) {
            table.push(
                [res[i].department_id, res[i].department_name, res[i].over_head_cost, res[i].total_sales, res[i].TotalProfit]
            );
        }
...
var createDepartment = function () {
    ...
    prompt.get(newDepartment, function (err, res) {
        ...
        connection.query('INSERT INTO Departments (department_name, over_head_cost, total_sales) VALUES (?, ?, ?);', [newDept[0].deptName, newDept[0].overHeadNew, newDept[0].autoTotalSales], function (err, result) {
            if (err) {
